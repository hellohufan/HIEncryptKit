...
#import "HIKeychain.h"
//#import "UIDevice+HIAdd.h"
//#import "HIKitMacro.h"
#import <Security/Security.h>

#ifndef kSystemVersion
#define kSystemVersion [HIKeychainItem systemVersion]
#endif
...
static HIKeychainErrorCode HIKeychainErrorCodeFromOSStatus(OSStatus status) {
    switch (status) {
        case errSecUnimplemented: return HIKeychainErrorUnimplemented;
        case errSecIO: return HIKeychainErrorIO;
        case errSecOpWr: return HIKeychainErrorOpWr;
        case errSecParam: return HIKeychainErrorParam;
        case errSecAllocate: return HIKeychainErrorAllocate;
        case errSecUserCanceled: return HIKeychainErrorUserCancelled;
        case errSecBadReq: return HIKeychainErrorBadReq;
        case errSecInternalComponent: return HIKeychainErrorInternalComponent;
        case errSecNotAvailable: return HIKeychainErrorNotAvailable;
        case errSecDuplicateItem: return HIKeychainErrorDuplicateItem;
        case errSecItemNotFound: return HIKeychainErrorItemNotFound;
        case errSecInteractionNotAllowed: return HIKeychainErrorInteractionNotAllowed;
        case errSecDecode: return HIKeychainErrorDecode;
        case errSecAuthFailed: return HIKeychainErrorAuthFailed;
        default: return 0;
    }
}

static NSString *HIKeychainErrorDesc(HIKeychainErrorCode code) {
    switch (code) {
        case HIKeychainErrorUnimplemented:
            return @"Function or operation not implemented.";
        case HIKeychainErrorIO:
            return @"I/O error (bummers)";
        case HIKeychainErrorOpWr:
            return @"ile already open with with write permission.";
        case HIKeychainErrorParam:
            return @"One or more parameters passed to a function where not valid.";
        case HIKeychainErrorAllocate:
            return @"Failed to allocate memory.";
        case HIKeychainErrorUserCancelled:
            return @"User canceled the operation.";
        case HIKeychainErrorBadReq:
            return @"Bad parameter or invalid state for operation.";
        case HIKeychainErrorInternalComponent:
            return @"Inrernal Component";
        case HIKeychainErrorNotAvailable:
            return @"No keychain is available. You may need to restart your computer.";
        case HIKeychainErrorDuplicateItem:
            return @"The specified item already exists in the keychain.";
        case HIKeychainErrorItemNotFound:
            return @"The specified item could not be found in the keychain.";
        case HIKeychainErrorInteractionNotAllowed:
            return @"User interaction is not allowed.";
        case HIKeychainErrorDecode:
            return @"Unable to decode the provided data.";
        case HIKeychainErrorAuthFailed:
...
static HIKeychainAccessible HIKeychainAccessibleEnum(NSString *s) {
    if ([s isEqualToString:(__bridge NSString *)kSecAttrAccessibleWhenUnlocked])
        return HIKeychainAccessibleWhenUnlocked;
    if ([s isEqualToString:(__bridge NSString *)kSecAttrAccessibleAfterFirstUnlock])
        return HIKeychainAccessibleAfterFirstUnlock;
    if ([s isEqualToString:(__bridge NSString *)kSecAttrAccessibleAlways])
        return HIKeychainAccessibleAlways;
    if ([s isEqualToString:(__bridge NSString *)kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly])
        return HIKeychainAccessibleWhenPasscodeSetThisDeviceOnly;
    if ([s isEqualToString:(__bridge NSString *)kSecAttrAccessibleWhenUnlockedThisDeviceOnly])
        return HIKeychainAccessibleWhenUnlockedThisDeviceOnly;
    if ([s isEqualToString:(__bridge NSString *)kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly])
        return HIKeychainAccessibleAfterFirstUnlockThisDeviceOnly;
    if ([s isEqualToString:(__bridge NSString *)kSecAttrAccessibleAlwaysThisDeviceOnly])
        return HIKeychainAccessibleAlwaysThisDeviceOnly;
    return HIKeychainAccessibleNone;
}

static id HIKeychainQuerySynchonizationID(HIKeychainQuerySynchronizationMode mode) {
    switch (mode) {
        case HIKeychainQuerySynchronizationModeAny:
            return (__bridge id)(kSecAttrSynchronizableAny);
        case HIKeychainQuerySynchronizationModeNo:
            return (__bridge id)kCFBooleanFalse;
        case HIKeychainQuerySynchronizationModeYes:
...
static HIKeychainQuerySynchronizationMode HIKeychainQuerySynchonizationEnum(NSNumber *num) {
    if ([num isEqualToNumber:@NO]) return HIKeychainQuerySynchronizationModeNo;
    if ([num isEqualToNumber:@YES]) return HIKeychainQuerySynchronizationModeYes;
    return HIKeychainQuerySynchronizationModeAny;
}

@interface HIKeychainItem ()
@property (nonatomic, readwrite, strong) NSDate *modificationDate;
@property (nonatomic, readwrite, strong) NSDate *creationDate;
@end

@implementation HIKeychainItem
...
- (NSMutableDictionary *)queryDic {
    ...
    if (kiOS7Later) {
        dic[(__bridge id)kSecAttrSynchronizable] = HIKeychainQuerySynchonizationID(self.synchronizable);
    }
...
- (NSMutableDictionary *)dic {
    ...
    if (self.accessible) dic[(__bridge id)kSecAttrAccessible] = HIKeychainAccessibleStr(self.accessible);
...
- (instancetype)initWithDic:(NSDictionary *)dic {
    ...
    self.accessible = HIKeychainAccessibleEnum(dic[(__bridge id)kSecAttrAccessible]);
    self.synchronizable = HIKeychainQuerySynchonizationEnum(dic[(__bridge id)kSecAttrSynchronizable]);
    
    return self;
}

- (id)copyWithZone:(NSZone *)zone {
    HIKeychainItem *item = [HIKeychainItem new];
...
- (NSString *)description {
    ...
    [str appendString:@"HIKeychainItem:{\n"];
...
+ (NSString *)getPasswordForService:(NSString *)serviceName
    ...
    HIKeychainItem *result = [self selectOneItem:item error:error];
...
+ (BOOL)setPassword:(NSString *)password
    ...
    item.service = serviceName;
    item.account = account;
    HIKeychainItem *result = [self selectOneItem:item error:NULL];
    if (result) {
...
+ (BOOL)insertItem:(HIKeychainItem *)item error:(NSError **)error {
    ...
    if (status != errSecSuccess) {
        if (error) *error = [HIKeychain errorWithCode:status];
        return NO;
    }
...
+ (HIKeychainItem *)selectOneItem:(HIKeychainItem *)item error:(NSError **)error {
    if (!item.service || !item.account) {
        if (error) *error = [HIKeychain errorWithCode:errSecParam];
        return nil;
    }
    ...
    return [[HIKeychainItem alloc] initWithDic:dic];
}

+ (HIKeychainItem *)selectOneItem:(HIKeychainItem *)item {
    return [self selectOneItem:item error:NULL];
}

+ (NSArray *)selectItems:(HIKeychainItem *)item error:(NSError **)error {
    NSMutableDictionary *query = [item dic];
    ...
    if (CFGetTypeID(result) == CFDictionaryGetTypeID()) {
        dic = (__bridge NSDictionary *)(result);
        HIKeychainItem *item = [[HIKeychainItem alloc] initWithDic:dic];
...
+ (NSArray *)selectItems:(HIKeychainItem *)item {
    return [self selectItems:item error:NULL];
}

+ (NSError *)errorWithCode:(OSStatus)osCode {
    HIKeychainErrorCode code = HIKeychainErrorCodeFromOSStatus(osCode);
    NSString *desc = HIKeychainErrorDesc(code);
    NSDictionary *userInfo = desc ? @{ NSLocalizedDescriptionKey : desc } : nil;
    return [NSError errorWithDomain:@"com.ibireme.HIkit.keychain" code:code userInfo:userInfo];
}
